...
public class AuthController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(code = 400, message = "[error] access token과 제공되지 않는 oauth를 입력한 경우"),
            @ApiResponse(code = 404, message = "[error] 회원 가입이 안되어 있을 경우"),
            @ApiResponse(code = 502, message = "[error] OAuth 연결 실패"),
    })
    ...
    @ApiOperation("회원 가입")
    @ApiResponses({
            @ApiResponse(code = 201, message = "[success] 회원 가입 후 token header에 반환"),
            @ApiResponse(code = 409, message = "[error] 이미 동일한 OAuth로 회원가입이 되어있는 경우")
    })
